"""Read-only marketing_brief.list tool executor."""
...
from app.tools.marketing_tools import (
    MARKETING_BRIEF_LIST_DEFAULT_LIMIT,
    MARKETING_BRIEF_LIST_MAX_LIMIT,
    MARKETING_BRIEF_LIST_TOOL_NAME,
    MARKETING_FORBIDDEN_ARGUMENT_KEYS,
    format_marketing_brief_compact,
    parse_marketing_brief_status,
)
from app.tools.permissions import ToolExecutionMode


@dataclass(frozen=True)
class MarketingBriefListOptions:
    limit: int = MARKETING_BRIEF_LIST_DEFAULT_LIMIT
...
def _coerce_bool(value: object, *, field_name: str, default: bool) -> bool:
    ...
    raise ToolValidationError(
        f"marketing_brief.list {field_name} must be a boolean",
        tool_name=MARKETING_BRIEF_LIST_TOOL_NAME,
...
def _coerce_limit(value: object) -> int:
    if value is None:
        return MARKETING_BRIEF_LIST_DEFAULT_LIMIT
    if not isinstance(value, int):
        raise ToolValidationError(
            "marketing_brief.list limit must be an integer",
            tool_name=MARKETING_BRIEF_LIST_TOOL_NAME,
            original_error_type="InvalidToolArguments",
        )
    if value < 1:
        raise ToolValidationError(
            "marketing_brief.list limit must be at least 1",
            tool_name=MARKETING_BRIEF_LIST_TOOL_NAME,
            original_error_type="InvalidToolArguments",
        )
    return min(value, MARKETING_BRIEF_LIST_MAX_LIMIT)


def parse_marketing_brief_list_arguments(arguments: dict[str, Any]) -> MarketingBriefListOptions:
    for forbidden_key in MARKETING_FORBIDDEN_ARGUMENT_KEYS:
        if forbidden_key in arguments:
            raise ToolValidationError(
                f"marketing_brief.list does not accept argument: {forbidden_key}",
                tool_name=MARKETING_BRIEF_LIST_TOOL_NAME,
                original_error_type="InvalidToolArguments",
            )

    return MarketingBriefListOptions(
        limit=_coerce_limit(arguments.get("limit")),
        status=parse_marketing_brief_status(
...
class MarketingBriefListToolExecutor:
    ...
    async def execute(
        ...
        try:
            options = parse_marketing_brief_list_arguments(tool_call.arguments)
        ...
        rows = await self._briefs.list_by_project(
            context.owner_id,
            context.project_id,
        ...
        items = [format_marketing_brief_compact(row) for row in rows]
        return ToolExecutionResult(
            **base,
