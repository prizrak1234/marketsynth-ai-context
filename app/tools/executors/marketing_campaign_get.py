"""Read-only marketing_campaign.get tool executor."""
...
class MarketingCampaignGetOptions:
    campaign_id: UUID


def parse_marketing_campaign_get_arguments(
    arguments: dict[str, Any],
) -> MarketingCampaignGetOptions:
    for forbidden_key in MARKETING_FORBIDDEN_ARGUMENT_KEYS:
        if forbidden_key in arguments:
            raise ToolValidationError(
                f"marketing_campaign.get does not accept argument: {forbidden_key}",
                tool_name=MARKETING_CAMPAIGN_GET_TOOL_NAME,
    ...
    if not isinstance(raw_campaign_id, str) or not raw_campaign_id.strip():
        raise ToolValidationError(
            "marketing_campaign.get requires a non-empty campaign_id string",
            tool_name=MARKETING_CAMPAIGN_GET_TOOL_NAME,
            original_error_type="InvalidToolArguments",
        )
    try:
        ...
        raise ToolValidationError(
            "marketing_campaign.get campaign_id must be a valid UUID",
            tool_name=MARKETING_CAMPAIGN_GET_TOOL_NAME,
            original_error_type="InvalidToolArguments",
        ) from exc

    return MarketingCampaignGetOptions(campaign_id=campaign_id)


class MarketingCampaignGetToolExecutor:
    ...
    async def execute(
        ...
        try:
            options = parse_marketing_campaign_get_arguments(tool_call.arguments)
        ...
        row = await self._campaigns.get_by_id_for_project(
            options.campaign_id,
            context.owner_id,
            context.project_id,
        )
        if row is None:
            existing = await self._campaigns.get_by_id(options.campaign_id)
            if existing is None:
                return ToolExecutionResult(
                    **base,
                    status=ToolExecutionStatus.FAILED,
            ...
            return ToolExecutionResult(
                **base,
                status=ToolExecutionStatus.FAILED,
                reason="campaign_access_denied",
                error_payload={
                    "error_type": "CampaignAccessDenied",
                    "safe_message": "Marketing campaign access denied",
                    "reason": "campaign_access_denied",
                },
            )

        payload = format_marketing_campaign_safe(row)
        return ToolExecutionResult(
            **base,
            status=ToolExecutionStatus.SUCCEEDED,
            output_payload={"campaign": payload, "count": 1},
            reason=None,
        )
