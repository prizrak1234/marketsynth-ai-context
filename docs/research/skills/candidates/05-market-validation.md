# Skill Audit — Market Validation

## Metadata

| Field | Value |
|-------|-------|
| **Card ID** | `SKILL-AUDIT-005` |
| **Marketsynth ID** | `MS-SKILL-005` |
| **Candidate name** | Market Validation |
| **Source URL / repo** | Internal Marketsynth BIV + marketingskills `free-tools`, `marketing-plan`, `customer-research` (partial) |
| **Author / vendor** | Marketsynth internal + Corey Haines (reference) |
| **License** | MIT (external portions) |
| **Version reviewed** | BIV frozen slices; external refs main |
...
Determine whether a business idea has sufficient evidence-backed market demand before launch investment.
...
| Question | Answer |
|----------|--------|
| CWF step strengthened | Idea → Research → Verdict |
| User-visible result | **Core CWF.1 product** — already partially implemented via Business Idea Validator. |
...
**Core CWF.1 product** — already partially implemented via Business Idea Validator.

---

## 4. Methodology

Marketsynth: evidence-backed BIV runs, gap-directed coverage, verdict mapper. External skills: adjacent validation frameworks — not a single packaged skill.
...
- Verdict + evidence pack
- Next-step commercial branch
...
- XmlRiver/Firecrawl adapters
- LLM synthesis with Citation Contract
...
**Primary internal capability** — external skills are reference only. CWF.1a Launch Pack decision already wired.
...
Controlled via existing research operators and sanitization.
...
Marketsynth has automated tests (`test_cwf_1a_launch_pack_decision.py`, BIV suites). External evals irrelevant for core verdict path.

---

## 9. Legal

Internal code proprietary; external MIT reference only.

### Reuse decision split

Internal implementation: continue Adapt/enhance. External install: Reject.
...
| Decision | **Adapt** |
|----------|----------------|
| **Rationale** | Do not import external validation skill — extend internal BIV/subskills using external **methodology** where gaps exist. |
| **Required modifications** | - Document mapping from marketingskills validation patterns to BIV gap types
- Keep single verdict gate |
...
- `app/business_idea_validation/`
- `docs/phase_ai_*` BIV audits
- marketingskills free-tools / customer-research
