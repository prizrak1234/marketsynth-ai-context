...
Preferred production auth: `CODEX_API_KEY`.
Supported fallback for this project: if `CODEX_API_KEY` is empty, `codex exec` may use existing `codex login` auth from `~/.codex/auth.json`.
Do not claim that `CODEX_API_KEY` is strictly required unless the implementation actually enforces that.
Document the tradeoff clearly:
- `CODEX_API_KEY` is more portable and survives fresh-machine rebuilds more reliably
- `codex login` can work on the same machine for the same service user, but is less portable for headless rebuild flows
...
- `CODEX_API_KEY` is preferred for production, but if the implementation supports `~/.codex/auth.json`, do not stop installation only because the key is empty.
...
```env
# Leave empty if codex auth already exists in ~/.codex/auth.json for the same service user
CODEX_API_KEY=sk-...
OPENAI_API_KEY=sk-...
...
- headless Codex execution must prefer `CODEX_API_KEY`
- if `CODEX_API_KEY` is empty but local `codex login` auth exists in `~/.codex/auth.json`, the bot may still run and this must be documented as a supported fallback
...
If Codex cannot authenticate, prefer:

```bash
CODEX_API_KEY=sk-...
```

Do not declare success if the bot only works because of an interactive local login state.
...
5. Write `.env.example`.
6. Create `.env` using the provided secrets.
7. Run `npm install`.
8. If `better-sqlite3` needs repair, repair it.
9. Run `npm run build`.
...
11. Start CodexClaw.
12. Verify the process is alive.
13. If `ALLOWED_CHAT_ID` is empty, explicitly tell the user:
    - open Telegram
    - send `/start`
    - send `/chatid`
    - copy the returned id
14. If `ALLOWED_CHAT_ID` was missing at first, update `.env`, restart the service, and verify the allowlist is now active.
15. Tell the user the exact first normal message to send after bootstrap.
...
6. System `bubblewrap` is present at `/usr/bin/bwrap` or the fallback situation is explicitly verified.
7. CodexClaw starts successfully.
8. The user has a clear Telegram bootstrap path.
9. The user has a clear first normal test step.
