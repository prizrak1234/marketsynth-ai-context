"""Phase AI.112 — Funnel Architect specialist execution."""
...
@pytest.mark.asyncio
async def test_cannot_execute_funnel_architect_before_offer_strategist(
    client: TestClient,
    auth_headers: dict[str, str],
    db_session: AsyncSession,
) -> None:
    project_id = create_project(client, auth_headers, "AI.112 Funnel guard")
    ...
    funnel_index = task_index_for(run, MarketingSpecialistType.FUNNEL_ARCHITECT)
    response = execute_specialist(client, auth_headers, project_id, run["id"], funnel_index)
    assert response.status_code == 409
    assert "Offer Strategist" in conflict_message(response)


@pytest.mark.asyncio
async def test_funnel_architect_executes_with_structured_output(
    client: TestClient,
    auth_headers: dict[str, str],
    db_session: AsyncSession,
) -> None:
    project_id = create_project(client, auth_headers, "AI.112 Funnel happy")
    owner_id = await owner_id_for_project(db_session, project_id)
    plan_id = await create_approved_plan_with_specialists(
        db_session,
        owner_id,
    ...
    body = response.json()
    assert body["specialist"] == MarketingSpecialistType.FUNNEL_ARCHITECT.value

    output = client.get(
        f"/projects/{project_id}/marketing-specialist-outputs/{body['specialist_output_id']}",
        headers=auth_headers,
    ).json()
    assert output["output_type"] == "funnel_design"
    ...
    task = run_after["task_snapshots"][funnel_index]
    assert task["status"] == MarketingPlanExecutionTaskStatus.SPECIALIST_COMPLETED.value


@pytest.mark.asyncio
async def test_duplicate_funnel_architect_execution_rejected(
    client: TestClient,
    auth_headers: dict[str, str],
    db_session: AsyncSession,
) -> None:
    project_id = create_project(client, auth_headers, "AI.112 Funnel duplicate")
    owner_id = await owner_id_for_project(db_session, project_id)
    plan_id = await create_approved_plan_with_specialists(
        db_session,
        owner_id,
    ...
    funnel_index = task_index_for(run, MarketingSpecialistType.FUNNEL_ARCHITECT)
    second = execute_specialist(client, auth_headers, project_id, run["id"], funnel_index)
    assert second.status_code == 409


@pytest.mark.asyncio
async def test_funnel_architect_no_tools_or_content_assets(
    client: TestClient,
    auth_headers: dict[str, str],
    db_session: AsyncSession,
) -> None:
    project_id = create_project(client, auth_headers, "AI.112 Funnel safety")
    owner_id = await owner_id_for_project(db_session, project_id)
    plan_id = await create_approved_plan_with_specialists(
        db_session,
        owner_id,
