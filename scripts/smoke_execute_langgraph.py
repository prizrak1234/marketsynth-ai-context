#!/usr/bin/env python3
"""Safe smoke: LangGraph execute when enabled (no real webhooks)."""
...
def main() -> int:
    ...
    if os.environ.get("AGENT_EXECUTION_LANGGRAPH_ENABLED", "true").lower() in (
        "0",
        "false",
        "no",
    ):
        return skip("AGENT_EXECUTION_LANGGRAPH_ENABLED is false")

    if os.environ.get("AGENT_EXECUTION_FORCE_CLASSIC", "false").lower() in (
        "1",
        "true",
        "yes",
    ):
        return skip("AGENT_EXECUTION_FORCE_CLASSIC is true")
    ...
    status, run = request_json(
        ...
        f"{base}/agent-runs",
        api_key=api_key,
        body={"agent_id": agent["id"], "input_payload": {"prompt": "smoke langgraph"}},
    )
    ...
    status, executed = request_json(
        "POST",
        f"{base}/agent-runs/{run['id']}/execute?engine=langgraph",
        api_key=api_key,
    )
    ...
    if execution.get("engine") != "langgraph":
        return fail(f"expected execution.engine=langgraph, got {execution}")

    return ok(f"langgraph execute succeeded for run {run['id']}")
